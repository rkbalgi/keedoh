...
public class MessagingUtils {
	...
	public static String buildFlightKey(MessageData msgData) {
		...
		for (Field f : msgData.getMessage().getRequestFragment().getFields()) {
	...
	public static String toHtml(MessageData rqMsgData, MessageData rpMsgData) {
		...
		if (rqMsgData != null) {
			builder.append(String
					.format("<table border=\"1\" style=\"font-family:verdana;font-size:8px;color:blue\">\n<tr><td align=\"center\" colspan=\"2\">%s</td></tr>",
							"Request"));
			List<Field> fields = rqMsgData.getMessage().getRequestFragment()
					.getFields();
			for (Field f : fields) {
		...
		if (rpMsgData != null) {
			builder.append(String
					.format("<table border=\"1\" style=\"font-family:verdana;font-size:8px;color:blue\">\n<tr><td align=\"center\" colspan=\"2\">%s</td></tr>",
							"Response"));
			List<Field> fields = rpMsgData.getMessage().getRequestFragment()
					.getFields();
			for (Field f : fields) {
