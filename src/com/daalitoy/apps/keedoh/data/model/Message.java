...
public class Message extends Model {
	...
	private MessageFragment requestFragment;
	private MessageFragment responseFragment;
	private Spec spec;
	...
	public MessageFragment getRequestFragment() {
		return requestFragment;
	}

	public void setRequestFragment(MessageFragment requestFragment) {
		this.requestFragment = requestFragment;
	}

	public MessageFragment getResponseFragment() {
		return responseFragment;
	}

	public void setResponseFragment(MessageFragment responseFragment) {
	...
	public static List<Message> allMessages(Spec spec) {

		try {
			String sql = "select * from msg where msg_msg_spec_id=?";
			PreparedStatement st = Mahout.getConnection().prepareStatement(sql);
			st.setInt(1, spec.getSpecId());
			ResultSet rs = st.executeQuery();
			List<Message> messages = Lists.newArrayList();
			while (rs.next()) {
				String msgName = rs.getString("MSG_NAME");
				Message msg = new Message(spec, msgName);
				msg.setMsgId(rs.getInt("MSG_ID"));
				msg.setRequestFragment(MessageFragment.fromDb(spec,
						rs.getInt("RQ_FRAG_ID")));
				msg.setResponseFragment(MessageFragment.fromDb(spec,
						rs.getInt("RP_FRAG_ID")));
				msg.setMtiValues(rs.getString("MSG_MTI_VAL"));
				messages.add(msg);
			}
			rs.close();
			st.close();
		...
		if (msg.getMsgId() == -1) {
			...
			try {
				...
				PreparedStatement st = Mahout.getConnection().prepareStatement(
						sql);
				st.setInt(1, msg.getRequestFragment().getFragmentId());
				st.setInt(2, msg.getResponseFragment().getFragmentId());
				st.executeUpdate();
				st.close();
