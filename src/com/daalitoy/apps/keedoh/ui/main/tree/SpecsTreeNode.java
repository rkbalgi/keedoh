...
public class SpecsTreeNode extends KeedohMutableTreeNode implements CanPopUp,
		ActionListener {
	...
	private void setupPopupMenu() {
		JMenuItem item = UIHelper.newJMenuItem("New", KeyEvent.VK_N,
				this, "__new__");
		popup.add(item);
	...
	public void actionPerformed(ActionEvent arg0) {
		if (arg0.getActionCommand().equals("__new__")) {
			SimpleTextInputDialog dialog = new SimpleTextInputDialog(
					getOwningTree(), "New Specification");
			String interfaceName = dialog.showDialog("Specification Name");
			if (interfaceName != null) {
				// add it to the db
				Spec spec = Spec.newSpec(interfaceName);
				SpecTreeNode node = new SpecTreeNode(spec);
				addToSelf(node);
