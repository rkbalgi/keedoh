...
public class MessageInternalFrame extends JInternalFrame implements
		ActionListener {
	...
	public MessageInternalFrame(Message msg) {
		super("Specification: " + msg.getSpec().getSpecName() + " Message: "
				+ msg.getMsgName());
		this.msg = msg;
		initComponents();
		setClosable(true);
	...
	private void initComponents() {
		...
		JButton fromFileButton = UIHelper
				.newButton(null, "__load_file__", this);
		fromFileButton.setIcon(IconFactory
				.getIcon("folder-horizontal-open.png"));
		...
		JButton clearRequestButton = UIHelper.newButton(null, "__rq_clear__",
				this);
		clearRequestButton.setIcon(IconFactory
				.getIcon("ui-text-field-clear.png"));
		clearRequestButton.setToolTipText("Clear Request");

		JButton clearResponseButton = UIHelper.newButton(null, "__rp_clear__",
				this);
		clearResponseButton.setIcon(IconFactory
				.getIcon("ui-text-field-clear.png"));
		...
		requestMsgDataTable.getTableHeader().setFont(
				UIHelper.STANDARD_BOLD_FONT);

		responseMsgDataTable = new JTable();
		responseMsgDataTable
				.setModel(new EditableMessageTableModel(msg, false));
		responseMsgDataTable.setFont(UIHelper.STANDARD_FONT);
		responseMsgDataTable.getTableHeader().setFont(
				UIHelper.STANDARD_BOLD_FONT);
	...
	public void actionPerformed(ActionEvent arg0) {
		if (arg0.getActionCommand().equals("__sock__")) {
			ConnectorConfig config = new ConnectorSelectionDialog(this)
					.showDialog();
			if (config != null) {
			...
			if (trace != null) {
				...
				requestMsgDataTable.setModel(new EditableMessageTableModel(
						msgData));
			}
		} else if (arg0.getActionCommand().equals("__export__")) {
			File file = UIHelper.getUserSelectedFile(this, FILE_TYPE.TEXT,
					false);
			if (file != null) {
				MessageData msgData = ((EditableMessageTableModel) requestMsgDataTable
						.getModel()).getMsgData();
				try {
					byte[] assembledData = msgData.assemble(true);
					Files.write(Hex.toString(assembledData), file,
							Charsets.US_ASCII);
				} catch (Exception e) {
			...
			try {
				MessageData rpMsgData = MessagingManager
						.getInstance()
						.dispatch(
								((EditableMessageTableModel) requestMsgDataTable
										.getModel()).getMsgData());
				EditableMessageTableModel model = new EditableMessageTableModel(
						rpMsgData);
			...
			requestMsgDataTable.setModel(new EditableMessageTableModel(msg,
					true));
		} else if (arg0.getActionCommand().equals("__rp_clear__")) {
			responseMsgDataTable.setModel(new EditableMessageTableModel(msg,
					true));
		} else if (arg0.getActionCommand().equals("__load_file__")) {
			File file = UIHelper
					.getUserSelectedFile(this, FILE_TYPE.TEXT, true);
			if (file != null) {
				try {
					List<String> traces = Files.readLines(file,
							Charsets.US_ASCII);
					String tmp = new TraceSelectionDialog(this, traces)
							.getTrace();
					if (tmp != null) {
						MessageData msgData = MessageData.build(msg, tmp, true);
						requestMsgDataTable
								.setModel(new EditableMessageTableModel(msgData));
					}
				} catch (Exception e) {
					UIHelper.showErrorDialog(this,
							"unable to load traces from file", e);
				}
			}
		}
