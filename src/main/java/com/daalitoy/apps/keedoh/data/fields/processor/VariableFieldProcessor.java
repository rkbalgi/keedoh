...
public class VariableFieldProcessor implements FieldProcessor {
    ...
    public void read(Field field, MessageData msgData, ByteBuffer buffer) {
        ...
        if (log.isDebugEnabled()) {
            log.debug(String.format("field: %s, length-indicator:%d",
                    f.getFieldName(), len));
        }
    ...
    public void assemble(Field field, MessageData msgData,
        ...
        VariableField f = (VariableField) field;
        try {
            byte[] vli = getFieldLen(f,
                    msgData.getFieldData(f).getData().length);

            if (log.isDebugEnabled()) {
                log.debug(String.format("writing field:%s, vli:%s",
                        field.getFieldName(), Hex.toString(vli)));
    ...
    private byte[] getFieldLen(VariableField f, int length) {
        byte[] vli = VariableFieldUtils.getLenIndicator(
    ...
    public String toStringData(Field field, byte[] data) {
        if (data != null && data.length > 0) {
            String stringData = CharsetUtils.toString(field.getEncodingType(),
                    data);
            return (stringData);
        } else {
            return ("");
    ...
    public byte[] toBinaryData(Field field, String stringData) {
        if (stringData != null && stringData.length() > 0) {
            return (CharsetUtils
                    .fromString(field.getEncodingType(), stringData));
        } else {
            return (new byte[0]);
        }
    }
...
class VariableFieldUtils {
    ...
    public static byte[] getLenIndicator(ENCODING_TYPE encodingType,
