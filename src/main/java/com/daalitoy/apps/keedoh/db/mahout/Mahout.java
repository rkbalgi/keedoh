...
public class Mahout {
  ...
  private static final Logger log = LogManager.getLogger(Mahout.class);
  private static final String dbConnectionStr = Properties.getProperty("DB_URL", null);
  ...
  public static void main(String[] args) throws MahoutException, SQLException {
    ...
    ResultSet rs = c.createStatement().executeQuery("select * from fld_type");
    while (rs.next()) {
      System.out.println(rs.getString("fld_type_name") + ":" + rs.getString("fld_processor_class"));
    }
    c.close();
  }
