...
public class MessagingUtils {
  ...
  public static String buildFlightKey(MessageData msgData) {
    ...
    builder.append(
        String.format(
            "[%s][%s]",
            msgData.getMessage().getSpec().getSpecName(), msgData.getMessage().getMsgName()));
  ...
  private static void process(Field field, MessageData msgData, StringBuilder builder) {

    if (field.isFlightKey()) {
      builder.append(String.format("[%s]", msgData.getFieldData(field).getStringData()));
    }
  ...
  public static String toHtml(MessageData rqMsgData, MessageData rpMsgData) {
    ...
    if (rqMsgData != null) {
      builder.append(
          String.format(
              "<table border=\"1\" style=\"font-family:verdana;font-size:8px;color:blue\">\n"
                  + "<tr><td align=\"center\" colspan=\"2\">%s</td></tr>",
              "Request"));
      List<Field> fields = rqMsgData.getMessage().getRequestSegment().getFields();
      for (Field f : fields) {
      ...
      builder.append(
          String.format(
              "<table border=\"1\" style=\"font-family:verdana;font-size:8px;color:blue\">\n"
                  + "<tr><td align=\"center\" colspan=\"2\">%s</td></tr>",
              "Response"));
      List<Field> fields = rpMsgData.getMessage().getRequestSegment().getFields();
      for (Field f : fields) {
  ...
  private static void export(Field f, MessageData msgData, StringBuilder builder) {
    builder.append(
        String.format(
  ...
  private static void mergeValues(Field f, MessageData reqMsgData, MessageData respMsgData) {
