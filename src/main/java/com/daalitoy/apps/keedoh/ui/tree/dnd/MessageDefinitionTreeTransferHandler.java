...
public class MessageDefinitionTreeTransferHandler extends TransferHandler {
    ...
    private boolean shouldAcceptDropLocation(DropLocation location) {
        ...
        KeedohMutableTreeNode node = (KeedohMutableTreeNode) loc.getPath()
                .getLastPathComponent();

        return (node instanceof RequestFragmentTreeNode)
                || (node instanceof ResponseFragmentTreeNode);
    ...
    public boolean importData(TransferSupport supp) {
        ...
        try {
            KeedohMutableTreeNode node = (KeedohMutableTreeNode) t
                    .getTransferData(DATA_FLAVOR);
            // Fetch the drop location

            // Insert the data at this location

            JTree.DropLocation loc = (javax.swing.JTree.DropLocation) supp
                    .getDropLocation();
            if (loc.getChildIndex() == -1) {
                // insert at end
                KeedohMutableTreeNode parentNode = (KeedohMutableTreeNode) loc
                ...
                if (parentNode instanceof RequestFragmentTreeNode) {
                    ...
                    if (!fragment.contains(field)) {
                        ...
                        MessageFieldTreeNode newNode = new MessageFieldTreeNode(
