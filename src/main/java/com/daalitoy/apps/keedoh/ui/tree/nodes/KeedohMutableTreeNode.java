...
public class KeedohMutableTreeNode extends DefaultMutableTreeNode {
  ...
  protected void addNodeAt(
      MessageFieldTreeNode node, KeedohMutableTreeNode parentNode, int location) {
    DefaultTreeModel model = (DefaultTreeModel) getOwningTree().getModel();
  ...
  private FieldTreeNode getNodeForField(Field field) {
    FieldTreeNode node;
    DefaultTreeModel model = (DefaultTreeModel) getOwningTree().getModel();
    MessageSpecFieldDictTreeNode rootNode = (MessageSpecFieldDictTreeNode) model.getRoot();
