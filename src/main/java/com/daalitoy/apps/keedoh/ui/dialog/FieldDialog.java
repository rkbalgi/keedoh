...
public class FieldDialog extends JDialog implements ActionListener {
  ...
  private JButton btnCancel = UIHelper.newButton("Cancel", "__cancel__", this);
      ...
      case VARIABLE:
        {
          ...
          tfIndicatorLength.setText(Integer.toString(vField.getIndicatorLength()));
          cbIndicatorEncoding.setSelectedItem(vField.getIndicatorEncodingType());
          cbVarFieldEncoding.setSelectedItem(field.getEncodingType());
          break;
        }
      case TERMINATED:
        {
          tfTerminatorCharacter.setText(
              Integer.toHexString(((TerminatedField) field).getTerminator()));
      ...
      case BITMAPPED:
        {
          ...
          // cbBmpFieldType.setSelectedItem(((BitmappedField) field).getBmpFieldType());
          break;
        }
    }
  }
  ...
  private void createBitmappedField() {
    ...
    if (!edit) field = new BitmappedField();
    else field = (BitmappedField) this.field;
    ...
    field.setEncodingType((ENCODING_TYPE) cbBmpFieldEncoding.getSelectedItem());
  ...
  private void createTerminatedField() {
    ...
    if (!edit) field = new TerminatedField();
    else field = (TerminatedField) this.field;

    field.setFieldName(tfFieldName.getText());
    field.setFieldType(FIELD_TYPE.TERMINATED);
    field.setFlightKey(cxFlightKey.isSelected());
    field.setMti(cxMti.isSelected());
    field.setTerminator((byte) Integer.parseInt(tfTerminatorCharacter.getText(), 16));
    field.setEncodingType((ENCODING_TYPE) cbTerminatedFieldEncoding.getSelectedItem());
  ...
  private void createVariableField() {
    ...
    if (!edit) field = new VariableField();
    else field = (VariableField) this.field;
    ...
    field.setMti(cxMti.isSelected());
    field.setSequence(Integer.parseInt(tfFieldSequence.getText()));
    field.setIndicatorLength(Integer.parseInt(tfIndicatorLength.getText().trim()));
    field.setIndicatorEncodingType((ENCODING_TYPE) cbIndicatorEncoding.getSelectedItem());

    field.setEncodingType((ENCODING_TYPE) cbVarFieldEncoding.getSelectedItem());
  ...
  private void createFixedField() {
    ...
    if (!edit) field = new FixedField();
    else field = (FixedField) this.field;
    ...
    field.setEncodingType((ENCODING_TYPE) cbFixedFieldEncoding.getSelectedItem());
    if (cbParent.getSelectedItem().equals("NONE")) {
      field.setParent(null);
    } else {
      field.setParent(((Field) cbParent.getSelectedItem()));
